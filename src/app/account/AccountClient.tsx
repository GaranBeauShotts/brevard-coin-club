...
export default function AccountClient({ userId, userEmail }: Props) {
  // profile
  ...
  useEffect(() => {
    // userId is guaranteed by the server wrapper, but this keeps things safe.
    if (!userId) return;
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setPreferredContact(e.target.value as "email" | "phone")
              }
              className="mt-1 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 outline-none focus:border-white/20"
              disabled={profileLoading || profileSaving}
            >
